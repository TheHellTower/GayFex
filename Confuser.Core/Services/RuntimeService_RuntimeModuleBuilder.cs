...
namespace Confuser.Core.Services {
	internal partial class RuntimeService {
		private sealed class RuntimeModuleBuilder : IRuntimeModuleBuilder {
			...
			void IRuntimeModuleBuilder.AddImplementation(string targetFrameworkName, Func<Stream> assemblyStreamFactory,
				...
				if (string.IsNullOrWhiteSpace(targetFrameworkName))
					throw new ArgumentException("Framework identifier must not be empty or white-space only.",
						nameof(targetFrameworkName));
				...
				try {
					...
					throw new ArgumentException("Failed to identify framework: " + targetFrameworkName,
						nameof(targetFrameworkName), ex);
				}

				if (!ImmutableInterlocked.TryAdd(ref _implementationFactories, framework,
					new Implementation(framework, assemblyStreamFactory, debugSymbolStreamFactory)))
					throw new ArgumentException(
						"There is already a implementation for the framework: " + targetFrameworkName,
						nameof(targetFrameworkName));
			}

			internal (NuGetFramework ActualFramework, Func<Stream> AssemblyStreamFactory, Func<Stream>
				DebugSymbolStreamFactory) GetImplementationFactory(NuGetFramework requestedFramework) {
				...
				if (nearestMatch == null)
					throw new ArgumentException("Runtime not implemented for: " + requestedFramework,
			...
			private sealed class Implementation : IFrameworkSpecific {
				...
				public Implementation(NuGetFramework targetFramework, Func<Stream> assemblyStreamFactory,
					Func<Stream> debugSymbolStreamFactory) {
					TargetFramework = targetFramework ?? throw new ArgumentNullException(nameof(targetFramework));
					AssemblyStreamFactory = assemblyStreamFactory ??
					                        throw new ArgumentNullException(nameof(assemblyStreamFactory));
					DebugSymbolStreamFactory = debugSymbolStreamFactory ??
					                           throw new ArgumentNullException(nameof(debugSymbolStreamFactory));
				}
