...
namespace Confuser.Optimizations.CompileRegex.Compiler {
	internal sealed class RegexRunnerFindFirstCharMethodCompiler : RegexRunnerMethodCompiler {
		...
		internal void GenerateFindFirstChar(RegexOptions options, RegexCode code) {
			...
			if (0 != (anchors & (RegexFCD.Beginning | RegexFCD.Start | RegexFCD.EndZ | RegexFCD.End))) {
				if (!rightToLeft) {
					if (0 != (anchors & RegexFCD.Beginning)) {
						var l1 = CreateLabel();
			...
			else if (bmPrefix != null && bmPrefix._negativeUnicode == null) {
